...
impl ContextCleanup for HaceController {
    fn cleanup_context(&mut self) {
...
impl HaceController {
    ...
    pub fn digest<const N: usize>(&self) -> [u32; N] {
        let ctx_ptr = Self::shared_ctx();
        let digest_ptr = unsafe { (*ctx_ptr).digest.as_ptr() as *const u32 };
        core::array::from_fn(|i| unsafe {
            // SAFETY: digest_ptr is valid for N words (N < 16 for supported algorithms)
            core::ptr::read_volatile(digest_ptr.wrapping_add(i))
        })
    }
}
