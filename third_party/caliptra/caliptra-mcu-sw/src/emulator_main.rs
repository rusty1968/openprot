...
//! socket traffic into the emulated target when built with one of its
//! `test-*` cargo features. Host-side tests drive firmware over the I3C TCP
//! socket (`--i3c-port`), so this entry point unconditionally starts the I3C
//! controller before entering the run loop.
//!
