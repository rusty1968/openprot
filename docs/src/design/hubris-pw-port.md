...
tasks/*.idol                → Service interface definitions
tasks/*/src/main.rs         → Task implementation (uses generated stubs)
build.rs                    → Idol code generation via idolatry
...
| **IPC Mechanism** | Idol-generated stubs over kernel IPC | Manual channel syscalls |
...
| **Shared Memory** | Leases (borrow/lend operations) | Static memory regions in system.json5 |
| **Peripheral Access** | Exclusive ownership via kernel | MPU-enforced memory regions |
| **Supervisor/Recovery** | Supervisor task (e.g., task-jefe reference impl) | No built-in supervisor (DIY) |
...
### Phase 1: Analysis
1. Inventory all Idol interfaces in your Hubris application
2. Document IPC call patterns and message flows
3. Map task dependencies and ownership relationships
4. Identify shared memory usage (leases)
5. Document peripheral access patterns
...
```rust
// Auto-generated by idolatry in build.rs
...
impl UartClient {
    pub fn write(&self, data: &[u8]) -> Result<(), UartError> {
        // Generated IPC call using kernel primitives
        sys_send(self.task_id, UART_OP_WRITE, data, &mut [])
            .map_err(|e| e.into())
    }

    pub fn read(&self, buffer: &mut [u8]) -> Result<usize, UartError> {
        let mut reply_buf = [0u8; 8];
        sys_send(self.task_id, UART_OP_READ, &[], &mut reply_buf)?;
        // Deserialize response
        Ok(usize::from_le_bytes(reply_buf[..8].try_into().unwrap()))
    }
}
```
...
// Auto-generated dispatch loop
fn main() -> ! {
    let mut driver = MyUartDriver::new();
    loop {
        idol_runtime::dispatch(&mut driver);  // Handles IPC internally
    }
}
```
