    ...
    B: I2c<SevenBitAddress>,
{
    ...
    for i in 0..op_count {
        ...
        let Ok(desc) = zerocopy::Ref::<_, I2cOpDesc>::from_bytes(&request[off..off + I2cOpDesc::SIZE])
        else {
            return encode_error(response, I2cError::InvalidOperation);
        };
        match desc.op_kind() {
    ...
    for i in 0..op_count {
        ...
        let desc =
            zerocopy::Ref::<_, I2cOpDesc>::from_bytes(&request[off..off + I2cOpDesc::SIZE]).unwrap();
        let len = desc.length();
        match desc.op_kind() {
            Ok(I2cOpKind::Write) => {
                ops[i] = Operation::Write(&request[w_off..w_off + len]);
                w_off += len;
            }
            Ok(I2cOpKind::Read) => {
                let (head, tail) = read_rem.split_at_mut(len);
                ops[i] = Operation::Read(head);
                read_rem = tail;
            }
