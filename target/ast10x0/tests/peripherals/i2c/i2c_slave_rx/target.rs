...
fn run_slave_rx_test() -> Result<(), &'static str> {
    ...
    let board = Ast10x0Board::new(Ast10x0BoardDescriptor {
        ...
        i2c_buses: &[I2cBusCfg { bus: 2, config: SLAVE_CFG }],
    });
    ...
    let mut driver =
        unsafe { i2c_backend::open_bus(2, &SLAVE_CFG) }.map_err(|e| {
    ...
    driver
        ...
        .map_err(|e| {
            pw_log::error!("configure_slave_address failed: {}", i2c_error_str(e) as &str);
    ...
    pw_log::info!("SLAVE READY addr=0x{:02x} — start external master now", SLAVE_ADDR as u32);
    ...
    if received != EXPECTED_PAYLOAD {
        pw_log::error!(
            ...
            received[0] as u32, received[1] as u32,
            received[2] as u32, received[3] as u32,
            EXPECTED_PAYLOAD[0] as u32, EXPECTED_PAYLOAD[1] as u32,
            EXPECTED_PAYLOAD[2] as u32, EXPECTED_PAYLOAD[3] as u32,
        );
